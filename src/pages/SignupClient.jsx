...
export default function SignupClient() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', country: '', region: '', city: '' });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const result = await signup({ ...form, role: 'client' });
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16 bg-brand-50/40">
      <div className="bg-white rounded-2xl shadow-soft p-8 max-w-md w-full border border-brand-100/60">
        <h1 className="font-display text-2xl font-semibold text-ink-900 mb-1">Create your client account</h1>
        <p className="text-ink-900/60 mb-6 text-sm">Book stylists, barbers and wig pros near you, anywhere in the world.</p>

        {error && <p className="bg-brand-50 text-brand-700 text-sm rounded-lg px-3 py-2 mb-4">{error}</p>}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-ink-900/80 mb-1.5">Full name</label>
          ...
          <button type="submit" className="w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold py-2.5 rounded-xl transition-colors">Create account</button>
        </form>
